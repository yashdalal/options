...
export function OptionsScreener({ onLogout, onLoginRequired }: OptionsScreenerProps) {
  ...
  const [marginProgress, setMarginProgress] = useState<{ done: number; total: number } | null>(
    null,
  );
  ...
  const loadMargins = useCallback(
    async (rows: ScreenCandidate[]) => {
      ...
      setMarginProgress({ done: 0, total: rows.length });
      const chunkSize = 5;
      const enriched = new Map<string, EnrichedCandidate>(
        rows.map((row) => [row.instrumentToken, { ...row, marginLoading: true }]),
      );
      ...
      for (let index = 0; index < rows.length; index += chunkSize) {
        ...
        try {
          ...
          if (!response.ok) {
            for (const row of chunk) {
              const current = enriched.get(row.instrumentToken);
              if (current) {
                enriched.set(row.instrumentToken, {
                  ...current,
                  marginLoading: false,
            ...
            const payload = (await response.json()) as {
              margins: { instrumentToken: string; margin: number | null; error?: string }[];
            };
            for (const result of payload.margins) {
              const current = enriched.get(result.instrumentToken);
              if (!current) {
                continue;
              }
              ...
              enriched.set(result.instrumentToken, {
                ...current,
        ...
        setMarginProgress({
          done: Math.min(rows.length, index + chunk.length),
          total: rows.length,
        });
      }
    },
  ...
  const runScreen = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        spreadMax: String(settings.spreadMax),
        returnMin: String(settings.returnMin),
        side: settings.side,
        lots: String(settings.lots),
        expenses: String(settings.expenses),
      });
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 p-4 sm:p-6">
      <header className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-sm text-zinc-600">
              Pick one company and expiry. Phase A filters by spread; Phase B lazy-loads Kotak
              margin for annualized return.
            </p>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
          ...
          <label className="flex flex-col gap-1 text-sm text-zinc-700">
            ...
            <input
              type="number"
              value={settings.spreadMax}
              onChange={(event) =>
                setSettings((current) => ({
                  ...current,
                  spreadMax: Number(event.target.value),
                }))
              }
              className="rounded-lg border border-zinc-300 px-2 py-1.5"
            />
          ...
          <label className="flex flex-col gap-1 text-sm text-zinc-700">
            ...
            <input
              ...
              onChange={(event) =>
                setSettings((current) => ({
                  ...current,
                  expenses: Math.max(0, Number(event.target.value) || 0),
                }))
              }
              className="rounded-lg border border-zinc-300 px-2 py-1.5"
            />
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
        <span>Spot: {formatNumber(snapshot?.spot)}</span>
        <span>Days left: {snapshot?.calendarDaysLeft ?? "—"}</span>
        <span>Candidates: {candidates.length}</span>
        {marginProgress ? (
          <span>
            Margin {marginProgress.done}/{marginProgress.total}
          </span>
        ) : null}
        <span>
          Updated:{" "}
          ...
          <tbody>
            {candidates.length === 0 ? (
              ...
              candidates.map((row, index) => {
                const qualifies = row.meetsSpread && row.meetsReturn === true;
                return (
                  <tr
                  ...
                  >
                    ...
                    <td className="border-b border-zinc-100 px-3 py-2">
                      {formatNumber(row.netPremium)}
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">
                      {row.calendarDaysLeft}
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">
                      {row.marginLoading
                        ? "…"
                    ...
                    <td
                      className={`border-b border-zinc-100 px-3 py-2${
                        qualifies ? " font-semibold text-emerald-800" : ""
                      }`}
                    >
                      {row.marginLoading ? "…" : formatPercent(row.annualizedReturnPct)}
                    </td>
                  </tr>
                );
              })
